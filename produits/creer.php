<?php
// Headers requis
header("Access-Control-Allow-Origin: *");    //authorise toutes les application (sites) a consommer notre api
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(!empty($donnees->nom) && !empty($donnees->description) && !empty($donnees->prix) && !empty($donnees->categories_id))
    {
        ...
        if($produit->creer()){
         ...
         http_response_code(503);
            echo json_encode(["message" => " Error!! tous les champs requises ne sont pas renseigner"]);   
    }
}else{
    // On gère l'erreur
